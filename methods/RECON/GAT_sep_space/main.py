...
def parse_args():
    ...
    args.add_argument("-outfolder", "--output_folder",
                      default="../entity_context/models/", help="Folder name to save the models.")
    ...
    args.add_argument("-neg_s_conv", "--valid_invalid_ratio_conv", type=int, default=40,
                      help="Ratio of valid to invalidtriples for convolution training")
    ...
    args.add_argument("-ctx_file", "--entities_context_data_file", type=str,
                      default='../entity_context/knowledge_graph.json', help="The path of the file in which to save the vocab")
    args.add_argument("-wrd_emb_dim", "--word_embed_dim", type=int,
                      default=300, help="The embedding dimension of the word vector")
...
def load_word_embedding(embed_file, vocab):
    ...
    with open(embed_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if len(parts) < word_embed_dim + 1:
                continue
            word = " ".join(parts[:-300])
            if word in vocab and vocab[word] >= word_min_freq and word not in already_used:
                already_used.add(word)
                vec = [np.float32(val) for val in parts[-300:]]
    ...
    embed_matrix.append(np.random.uniform(-0.25, 0.25, word_embed_dim))
    embed_vocab["ALL_ZERO"] = word_idx
    word_idx += 1

    embed_matrix.append(np.random.uniform(-0.25, 0.25, word_embed_dim))
    embed_vocab["_UNKNOWN"] = word_idx
    word_idx += 1
...
def train_entity_embeddings(args, word_embed_matrix, word_vocab, char_vocab):
    ...
    if CUDA:
      ...
      model_gat.load_state_dict(torch.load(
        os.path.join(args.output_folder,'trained_{}.pth'.format(args.epochs_gat-1)), map_location=torch.device('cpu')))
    ...
    for epoch in range(args.epochs_gat):
        ...
        save_model(model_entity_embedding, args.data, epoch,
...
def evaluate_conv(args, unique_entities_train, unique_entities_test):
    ...
    model_gat = SpKBGATModified(entity_embeddings, relation_embeddings, args.entity_out_dim, args.entity_out_dim,
                                args.drop_GAT, args.alpha, args.nheads_GAT, initial_entity_emb_params)
    model_gat.load_state_dict(torch.load(
        '{0}trained_{1}.pth'.format(args.output_folder, args.epochs_gat-1)))
...
def save_entity_relation_final_embeddings(args):
    model_gat = SpKBGATModified(entity_embeddings, relation_embeddings, args.entity_out_dim, args.entity_out_dim,
                                args.drop_GAT, args.alpha, args.nheads_GAT, initial_entity_emb_params)
    model_gat.load_state_dict(torch.load(
        '{0}trained_{1}.pth'.format(args.output_folder, args.epochs_gat-1)))
...
save_entity_relation_final_embeddings(args)
