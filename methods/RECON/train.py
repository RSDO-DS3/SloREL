...
def parse_args():
    ...
    args.add_argument("-sf", "--save_folder",
                      default="./trained_models/", help="folder to save the models")
...
def train():
    ...
    if "RECON" in model_name:
        context_data_file = "entity_context/knowledge_graph.json"
    if True:
        gat_embedding_file = 'entity_context/models/final_entity_embeddings.json'
        gat_entity2id_file = 'entity_context/entity2id.txt'
    if model_name == "RECON":
        # Point to the trained model/embedding/data files
        gat_relation_embedding_file = 'entity_context/models/final_relation_embeddings.json'
        gat_relation2id_file = 'entity_context/relation2id.txt'
        w_ent2rel_all_rels_file = 'entity_context/models/W_ent2rel.json.npy'
    ...
    training_data, _ = io.load_relation_graphs_from_file(data_folder + train_set, load_vertices=True, data='wikidata')
    ...
    val_data, _ = io.load_relation_graphs_from_file(data_folder + val_set, load_vertices=True, data="wikidata")
    ...
    if property_index:
        ...
        _, property2idx = embedding_utils.init_random({e["kbID"] for g in training_data
                                                       for e in g["edgeSet"]} | {"P0"}, 1, add_all_zeroes=True,
                                                      add_unknown=True)
    ...
    idx2property = dict()
    for rel in property2idx:
        idx2property[property2idx[rel]] = rel
    ...
    train_as_indices = list(
        graphs_to_indices(training_data, word2idx, property2idx, max_sent_len, embeddings=embeddings,
    ...
    val_as_indices = list(graphs_to_indices(val_data, word2idx, property2idx, max_sent_len, embeddings=embeddings,
                                            position2idx=position2idx, entity2idx=entity2idx))
    val_data = None
    print(val_as_indices[0].shape)
    ...
    opt = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate,
    ...
    for train_epoch in range(model_params['nb_epoch']):
        ...
        for i in tqdm(range(int(train_as_indices[0].shape[0] / model_params['batch_size']))):
            ...
            if "RECON" in model_name:
                ...
                unique_entities, unique_entities_surface_forms, max_occurred_entity_in_batch_pos = context_utils.get_batch_unique_entities(
                    train_as_indices[4][indices[i * model_params['batch_size']: (i + 1) * model_params['batch_size']]],
                    train_as_indices[5][indices[i * model_params['batch_size']: (i + 1) * model_params['batch_size']]])
            ...
            if model_name == "RECON-EAC-KGGAT":
                gat_entity_embeddings = context_utils.get_gat_entity_embeddings(entity_indices, entity2idx, idx2entity,
                                                                                gat_entity2idx, gat_embeddings)
            elif model_name == "RECON":
            ...
            if model_name == "RECON":
                ...
                output = model(Variable(torch.from_numpy(sentence_input.astype(int))).cuda(),
                               Variable(torch.from_numpy(entity_markers.astype(int))).cuda(),
                               Variable(torch.from_numpy(np.array(train_as_indices[3][
            ...
            loss = loss_func(output, Variable(torch.from_numpy(labels.astype(int)).long()).view(-1).cuda())
        ...
        for i in tqdm(range(int(val_as_indices[0].shape[0] / model_params['batch_size']))):
            ...
            if "RECON" in model_name:
                ...
                unique_entities, unique_entities_surface_forms, max_occurred_entity_in_batch_pos = context_utils.get_batch_unique_entities(
                    val_as_indices[4][i * model_params['batch_size']: (i + 1) * model_params['batch_size']],
                    val_as_indices[5][i * model_params['batch_size']: (i + 1) * model_params['batch_size']])
                unique_entities_context_indices = context_utils.get_context_indices(unique_entities,
                                                                                    unique_entities_surface_forms,
                                                                                    context_data, idx2entity, word2idx,
                                                                                    char_vocab,
