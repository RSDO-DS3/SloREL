...
def test():
    ...
    model = model.cuda()
    model.load_state_dict(torch.load(save_folder + trained_model))

    print("Testing")

    print("Results on the test set")
    print(data_folder + test_set)
    test_set, _ = io.load_relation_graphs_from_file(data_folder + test_set, load_vertices=True, data='wikidata')
    ...
    for i in tqdm(range(int(test_as_indices[0].shape[0] / model_params['batch_size']))):
        ...
        if model_name == "RECON":
                ...
                output = model(Variable(torch.from_numpy(sentence_input.astype(int))).cuda(),
                               Variable(torch.from_numpy(entity_markers.astype(int))).cuda(),
                               Variable(torch.from_numpy(np.array(test_as_indices[3][
        ...
        if (model_name != "LSTM" and model_name != "PCNN" and model_name != "CNN"):
            ...
            #token_positions = test_as_indices[7][i * model_params['batch_size']: (i + 1) * model_params['batch_size']]
            #token_positions = reduce(lambda x, y: x + y, token_positions)
        else:
            entity_pairs = test_as_indices[6][i * model_params['batch_size']: (i + 1) * model_params['batch_size']]
            #token_positions = test_as_indices[7][i * model_params['batch_size']: (i + 1) * model_params['batch_size']]

        start_idx = i * model_params['batch_size']
        for index, (i, j, entity_pair) in enumerate(zip(score, labels, entity_pairs)):  # token_positions
            sent = ' '.join(test_set[start_idx + indices2[index] // (
                        model_params['max_num_nodes'] * (model_params['max_num_nodes'] - 1))]['tokens']).strip()
            result_file.write("{} | {} | {} | {} | {} | {} | {} | {}\n".format(sent, entity_pair[0], entity_pair[1],
                                                                               "token_position[0]", "token_position[1]",
                                                                     idx2property[pred_labels[index]],
                                                                     idx2property[labels[index]],
