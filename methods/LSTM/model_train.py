...
if __name__ == "__main__":
    ...
    if not path.exists("trainedmodels"):
        makedirs("trainedmodels")
    ...
    if "train" in mode:
        ...
        if "continue" in mode:
            ...
            model.load_weights("trainedmodels")
        ...
        callback_history = model.fit(train_as_indices[:-1],
                                     ...
                                     callbacks=[callbacks.EarlyStopping(monitor="val_loss", patience=5, verbose=1),
                                                callbacks.ModelCheckpoint(
                                                    "trainedmodels/model_ContextWeighted.kerasmodel",
        ...
        print("Saving trials.")
        with open("../data/trials/" + "model_ContextWeighted" + "_final_trails.json", 'w') as ftf:
            json.dump([(t['misc']['vals'], t['result']) for t in trials.trials], ftf)

    print("Loading the best model")
    model = getattr(keras_models, "model_ContextWeighted")(keras_models.model_params, embedding_matrix, max_sent_len, n_out)
